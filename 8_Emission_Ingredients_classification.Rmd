...
ingredients <- readRDS("~/ERL/Emission_Analysis/data/ingredients_harmonized_subset.rds")
merged_metadata <- readRDS("~/ERL/Emission_Analysis/data/merged_metadata_nutrition_inference.rds")
diet_percentages <- read.csv("~/ERL/Emission_Analysis/data/diet_percentages.csv")
...
Ing_classif <- Ing_classif %>%
  dplyr::filter(!.data$D.Item %in% c("Salt", "Water"))
...
csv_path <- "data/Ingredients_nutrition_unmatched_GPT.csv"  # <- change if saved elsewhere
...
library(dplyr)
# ─────────────────────────────────────────────────────────────────────────────
# Reclassify ONLY unmatched diets, then merge back
# Requires in env:
#   unmatched_diet (B.Code, D.Item ...), diet_percentage, canon_ing, classif_ing,
#   classif_diet (initial classifications), canonicalize_item(), method1..method7()
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Coverage & method breakdown (with consistent keys)
#   Uses: diet_percentages, nutr_diet, classif_diet, classif_diet_2nd
#   Assumes: canonicalize_item() exists; if not, define a minimal one here
# ─────────────────────────────────────────────────────────────────────────────

# Fallback canonicalizer if not already defined
if (!exists("canonicalize_item")) {
  clean_key <- function(x) gsub("\\s+", " ", trimws(tolower(x)))
  canonicalize_item <- function(x) {
    x <- clean_key(x)
    x <- gsub("^unspecified\\s+molasses$", "molasses", x)
    x <- gsub("\\bbran\\b", "offal", x)
    x
  }
}

# --- Universe of diets (union of % table and nutrient table), canonicalized ---
...
# --- Final method breakdown (use the final table; keys canonicalized) --------
...
# --- Optional: quick diagnostics for key mismatches --------------------------
...
```{r}
write.csv(ingredients, "~/ERL/Emission_Analysis/data/ingredients_classified.csv")
write.csv(classif_diet, "~/ERL/Emission_Analysis/data/classif_diet.csv")
write.csv(unmatched_diet, "~/ERL/Emission_Analysis/data/unmatched_diet.csv")
write.csv(classif_ing_simple, "~/ERL/Emission_Analysis/data/classif_ing_simple.csv")

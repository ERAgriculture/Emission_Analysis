...
classify_diet_practice <- function(
  ...
  ctrl_key,
  classif_ing_simple,
  TOTAL_TOL = 0.05,   # ±5% change in total diet mass
  ING_TOL   = 1.0,    # ≥1% change to count at item/category level
  BAL_TOL   = 3.0     # “balanced” window for substitutions
){
  ...
  ing_lu <- classif_ing_simple %>%
    ...
    summarise(
      Ingredient_Category_lu = dplyr::first(na.omit(Ingredient_Category_lu)),
      .groups = "drop"
    )
  ...
  # ---------------- Category-level deltas (per pair) ----------------
  cat_delta <- trt_vs_ctrl %>%
    group_by(B.Code, Control, Trt, Ingredient_Category) %>%
    summarise(
      cat_pos = sum(pmax(dP, 0), na.rm = TRUE),
      cat_neg = sum(pmax(-dP, 0), na.rm = TRUE),
      .groups = "drop"
    )
  ...
  cat_summ <- cat_delta %>%
    ...
    summarise(
      cat_pos_total = sum(cat_pos, na.rm = TRUE),
      cat_neg_total = sum(cat_neg, na.rm = TRUE),
      n_cats_pos    = sum(cat_pos > 0),
      n_cats_neg    = sum(cat_neg > 0),
      cats_added    = summarize_types(Ingredient_Category[cat_pos > 0]),
      cats_dropped  = summarize_types(Ingredient_Category[cat_neg > 0]),
      .groups = "drop"
    )

  # ---------------- Total diet mass change ----------------
  delta_totals <- pairs %>%
  ...
  # ---------------- Top ingredient changes (context) ----------------
  ...
  paired <- trt_vs_ctrl %>%
    ...
    summarise(
      ...
      ing_set_trt  = list(sort(unique(D.Item[in_trt]))),
      ing_set_ctrl = list(sort(unique(D.Item[in_ctrl]))),
      same_ingredient_set = setequal(ing_set_trt[[1]], ing_set_ctrl[[1]]),
      .groups = "drop"
    ) %>%
    left_join(cat_summ,   by = c("B.Code","Control","Trt")) %>%
    left_join(top_changes, by = c("B.Code","Control","Trt")) %>%
    mutate(
      # ---------- Classification ----------
      Practice = case_when(
        is.finite(ΔTOTAL) & abs(ΔTOTAL) >  TOTAL_TOL & POS_sum >= ING_TOL ~ "Addition",
        abs(ΔTOTAL) <= TOTAL_TOL & same_ingredient_set &
          cat_pos_total >= ING_TOL & cat_neg_total >= ING_TOL ~ "Addition",
        is.finite(ΔTOTAL) & abs(ΔTOTAL) <= TOTAL_TOL &
          cat_pos_total >= ING_TOL & cat_neg_total >= ING_TOL &
    ...
    rowwise() %>%
    mutate(
      # ---------- Build local substitution pairs safely ----------
      Substitution_pairs_detail = {
        if (Practice != "Substitution") {
          ""
        } else {
          this_code <- cur_data()$B.Code
          this_ctrl <- cur_data()$Control
          this_trt  <- cur_data()$Trt

          local <- cat_delta %>%
            filter(B.Code == this_code, Control == this_ctrl, Trt == this_trt)

          pos_type <- local %>% filter(cat_pos > 0) %>% arrange(desc(cat_pos))
          neg_type <- local %>% filter(cat_neg > 0) %>% arrange(desc(cat_neg))
    ...
    mutate(
      Subpractice = dplyr::case_when(
        Practice == "Addition" & n_cats_pos > 1 ~ "Mixed",
        Practice == "Addition" & n_cats_pos == 1 ~ cats_added,
        Practice == "Substitution" & stringr::str_detect(Substitution_pairs_detail, ";") ~ "Substitution (Mixed)",
        Practice == "Substitution" ~ Substitution_pairs_detail,
        TRUE ~ ""
      )
    ) %>%
    select(
      ...
      ΔTOTAL, POS_sum, NEG_sum, n_pos, n_neg,
      same_ingredient_set,
      Types_added = cats_added,
      Types_decreased = cats_dropped,
      Substitution_pairs_detail,
      Top_changes
    ) %>%
    ...
    mutate(
      Substitution_pairs = {
        if (Practice != "Substitution") {
          ...
          paste0(Types_decreased, " → ", Types_added)
        } else {
          bc <- B.Code; a <- A; b <- B
          pos_types <- A_vs_B %>%
...
diet_pairs <- diet_final %>%
  ...
  filter(lengths(all_arms) >= 2) %>%       # <— filter out single-diet codes
...
pairwise_diets <- diet_pairs %>%
  ...
  mutate(
    ...
    Subpractice = if_else(Diet_A != Diet_B, paste0(Diet_A, " → ", Diet_B), "")
  )
...
# --- 1️⃣ Identify papers with valid CH₄ emissions ----------------------------
...
# --- 2️⃣ Ingredient completeness by paper ------------------------------------
...
# --- 3️⃣ Diet classification coverage ----------------------------------------
...
# --- 4️⃣ Papers with at least one ingredient-level practice ------------------
paper_with_practice <- practice_tbl %>%
  distinct(B.Code) %>%
  mutate(has_practice = TRUE)

# --- 5️⃣ Papers with at least one diet shift (pairwise) ----------------------
paper_with_diet_shift <- pairwise_diet_shift_tbl %>%
  filter(Practice == "Diet shift") %>%
  distinct(B.Code) %>%
  mutate(has_diet_shift = TRUE)

# --- 6️⃣ Merge all metadata together -----------------------------------------
meta <- ge_with_ch4 %>%
  mutate(has_CH4 = TRUE) %>%     # 👈 add this line
...
# --- 7️⃣ Filter combinations of interest -------------------------------------
# 7a. Ingredient-level: emissions + all ingredients classified + ≥1 practice
ing_practice_emiss <- meta %>%
  filter(has_CH4, all_ing_classified, has_practice)

# 7b. Diet-level: emissions + at least 1 classified diet + ≥1 diet shift
diet_shift_emiss <- meta %>%
  filter(has_CH4, has_any_classified, has_diet_shift)

# --- 8️⃣ Summarise by product/species ----------------------------------------
...
cat("Total papers with ingredient practices + CH₄:", 
    n_distinct(ing_practice_emiss$B.Code), "\n")
cat("Total papers with diet shifts + CH₄:", 

...
### 4.2) Estimate total feed intake at the diet level to deduce missing ingredients intake. 
...
##       (does NOT affect diet_intake_all)
## ---------------------------------------------------------------------------
...
dmi_eval <- diet_intake_measured %>%
  ...
  mutate(
    ...
    abs_error_kg  = abs(error_kg),
    rel_error     = error_kg / diet_intake_kg,
...
dmi_eval_overall <- dmi_eval %>%
  summarise(
    ...
    bias_kg    = mean(error_kg, na.rm = TRUE),
    mae_kg     = mean(abs_error_kg, na.rm = TRUE),
    mape_pct   = mean(abs_rel_error, na.rm = TRUE) * 100
  )
...
```{r}
# ## ---------------------------------------------------------------------------
# ## 4) Use diet_intake_all to convert diets defined ONLY in relative units
# ##    (% / % Diet / g·kg⁻¹) where NO absolute amount exists.
# ##    These are typically the "we only have %" + no feed intake cases.
# ## ---------------------------------------------------------------------------
# 
# # Identify diets where:
# #  - we have diet_intake (measured OR predicted),
# #  - at least one relative unit (% or g/kg),
# #  - NO absolute (g or kg) ingredient amount yet.
# rel_only_diets <- ingredients %>%
#   left_join(diet_intake_all, by = c("B.Code", "A.Level.Name")) %>%
#   group_by(B.Code, A.Level.Name) %>%
#   summarise(
#     has_diet_intake = any(!is.na(diet_intake_kg)),
#     has_abs = any(D.Unit.Amount %in% c("g", "kg") & !is.na(D.Amount)),
#     has_rel = any(D.Unit.Amount %in% c("%", "% Diet", "g/kg") & !is.na(D.Amount)),
#     .groups = "drop"
#   ) %>%
#   filter(has_diet_intake, !has_abs, has_rel)
# 
# rel_key <- rel_only_diets %>%
#   mutate(key = paste(B.Code, A.Level.Name)) %>%
#   pull(key)
# 
# ingredients <- ingredients %>%
#   left_join(diet_intake_all, by = c("B.Code", "A.Level.Name")) %>%
#   mutate(
#     diet_key      = paste(B.Code, A.Level.Name),
#     use_rel_pred  = diet_key %in% rel_key
#   ) %>%
#   mutate(
#     # Convert relative units to kg/day using diet_intake_kg
#     D.Amount = case_when(
#       use_rel_pred & D.Unit.Amount %in% c("%", "% Diet") &
#         !is.na(D.Amount) & !is.na(diet_intake_kg) ~
#         (D.Amount / 100) * diet_intake_kg,                     # kg/d
# 
#       use_rel_pred & D.Unit.Amount == "g/kg" &
#         !is.na(D.Amount) & !is.na(diet_intake_kg) ~
#         (D.Amount * diet_intake_kg) / 1000,                    # g/kg * kg = g -> /1000 -> kg
# 
#       TRUE ~ D.Amount
#     ),
#     D.Unit.Amount = case_when(
#       use_rel_pred & D.Unit.Amount %in% c("%", "% Diet", "g/kg") &
#         !is.na(diet_intake_kg) ~ "kg",
#       TRUE ~ D.Unit.Amount
#     ),
#     # Flag provenance of these amounts
#     D.Type = dplyr::case_when(
#       use_rel_pred & !is.na(diet_intake_kg) &
#         (D.Unit.Amount == "kg") ~
#         ifelse(is.na(D.Type),
#                "from_predicted_DMI_Archimede2014",
#                paste0(D.Type, ";from_predicted_DMI_Archimede2014")),
#       TRUE ~ D.Type
#     )
#   ) %>%
#   select(-diet_key)
...
# ## ---------------------------------------------------------------------------
# ## 5) COMPLEMENT FILL:
# ##    For diets where we know total intake (measured or predicted)
# ##    and EXACTLY ONE non-mineral/salt/water ingredient still has NA D.Amount,
# ##    compute that missing amount as:
# ##      total_intake_kg - sum_known_ingredients_kg
# ## ---------------------------------------------------------------------------
# 
# # First, attach diet_intake_kg cleanly (avoid .x/.y suffixes)
# ingredients_with_dietintake <- ingredients %>%
#   # drop any previous diet_intake columns if they exist
#   dplyr::select(-dplyr::any_of(c("diet_intake_kg", "diet_intake_source"))) %>%
#   left_join(diet_intake_all, by = c("B.Code", "A.Level.Name"))
# 
# # Compute complement values at diet level
# complement_info <- ingredients_with_dietintake %>%
#   group_by(B.Code, A.Level.Name) %>%
#   summarise(
#     # take first non-NA diet_intake_kg, or NA if none
#     total_intake = {
#       x <- diet_intake_kg[!is.na(diet_intake_kg)]
#       if (length(x) == 0) NA_real_ else x[1]
#     },                                                  # kg/d
#     n_missing    = sum(is.na(D.Amount) & !ignore_for_balance),
#     missing_item = {
#       idx <- which(is.na(D.Amount) & !ignore_for_balance)
#       if (length(idx) == 0) NA_character_ else D.Item[idx[1]]
#     },
#     sum_known    = sum(D.Amount[!is.na(D.Amount) & !ignore_for_balance],
#                        na.rm = TRUE),
#     .groups      = "drop"
#   ) %>%
#   # Only fill when: we know total intake AND exactly one missing amount
#   filter(!is.na(total_intake), n_missing == 1) %>%
#   mutate(
#     complement_value = total_intake - sum_known,
#     complement_value = ifelse(complement_value < 0, 0, complement_value)  # avoid negative
#   )
# 
# # Merge complement back into the original `ingredients` table
# ingredients <- ingredients %>%
#   left_join(
#     complement_info %>%
...
#   ) %>%
#   mutate(
#     # Fill missing amount where complement_value is available
#     D.Amount = ifelse(is.na(D.Amount) & !is.na(complement_value),
#                       complement_value,
#                       D.Amount),
# 
#     # Standardise units for complemented amount
...
# # Clean helper column
# ingredients <- ingredients %>%
#   select(-ignore_for_balance)


###############################################################################
### END Archimède-based DMI + complement + evaluation block
###############################################################################

...
ingredients <- merged_metadata$Animals.Diet %>%
  ...
  filter(D.Unit.Amount!="Unspecified")%>% # most of the time for basal diets
  ...
  mutate(
    ...
    Out.Unit = case_when(
      B.Code == "NN0217" & str_detect(Out.Unit, "kg dm/individual/day") &  str_detect(Out.Code.Joined, "^Feed Intake") ~
        "g dm/kg body weight (0.75)/individual/day",
      TRUE ~ Out.Unit
    ),
...
# ── Sanity check on corrected records ─────────────────────────
merged_metadata$Data.Out %>%
  filter(B.Code %in% c("AG0051", "NN0212", "NN0217"),
         Out.Subind == "Feed Intake") %>%
  select(B.Code, A.Level.Name, ED.Mean.T, Out.Unit) %>%
  arrange(B.Code) %>%
  print(n = Inf)

...
### 11.1 Estimation of Enteric Methane Emissions Usi
ng IPCC Tier 2
...
# === 1. LOAD GE DATA ======================================================
Gross_energy_GE <- Gross_energy %>%
  filter(DC.Variable == "GE") %>%
  mutate(
    DC.Value = as.numeric(DC.Value),
    D.Item_clean = str_squish(str_to_lower(D.Item))
  )


# === 2. Helper function: collapse ONLY dried/dry/ground ===================

process_words <- c("dried", "dry", "ground")

make_base_item <- function(x) {
  x_clean <- x %>% str_to_lower() %>% str_squish()
  
  # ----- optional simple synonyms (safe, do NOT change nutrition) --------
  # stover vs straw
  x_clean <- str_replace_all(x_clean, "\\bstover\\b", "straw")
  # offal misspelling → bran
  x_clean <- str_replace_all(x_clean, "\\boffal\\b", "bran")
  # OPTIONAL: unify molasses names (if you want)
  # x_clean <- ifelse(str_detect(x_clean, "molasses"), "molasses", x_clean)
  
  # ----- drop only non-nutritional processes ------------------------------
  pattern <- paste0("\\b(", paste(process_words, collapse="|"), ")\\b")
  x_clean <- str_remove_all(x_clean, pattern)
...
Gross_energy_GE <- Gross_energy_GE %>%
  mutate(D.Item_base = make_base_item(D.Item_clean))


# === 3. Build source GE per base ingredient ===============================

GE_base_source <- Gross_energy_GE %>%
  filter(!is.na(DC.Value)) %>%
  group_by(D.Item_base) %>%
  summarise(
    n_source_rows = n(),
    GE_impute = mean(DC.Value, na.rm = TRUE),
    GE_min    = min(DC.Value, na.rm = TRUE),
    GE_max    = max(DC.Value, na.rm = TRUE),
    .groups = "drop"
  )


# === 4. Identify missing GE rows that CAN be imputed ======================

GE_missing_with_base <- Gross_energy_GE %>%
  filter(is.na(DC.Value)) %>%
  left_join(GE_base_source, by = "D.Item_base") %>%
  filter(!is.na(GE_impute))


# === 5. Diagnostics =======================================================

n_total_missing      <- Gross_energy_GE %>% filter(is.na(DC.Value)) %>% nrow()
n_imputable_rows     <- nrow(GE_missing_with_base)
n_imputable_items    <- n_distinct(GE_missing_with_base$D.Item)
n_imputable_papers   <- n_distinct(GE_missing_with_base$B.Code)

message("────────────────────────────────────────────")
message("Total missing GE rows: ", n_total_missing)
message("Imputable rows (dried/dry/ground): ", n_imputable_rows)
message("Distinct ingredients affected: ", n_imputable_items)
message("Distinct papers gaining GE: ", n_imputable_papers)
message("────────────────────────────────────────────")




# === 6. Impute GE for harmonisable ingredients ============================

Gross_energy_GE_imputed <- Gross_energy_GE %>%
  left_join(GE_base_source, by = "D.Item_base") %>%
  mutate(
    DC.Value = if_else(
      is.na(DC.Value) & !is.na(GE_impute),
      GE_impute,
      DC.Value
    ),
    GE_source = case_when(
      is.na(GE_source) & is.na(DC.Value) & !is.na(GE_impute) ~ "harmonized_dried_ground",
      TRUE ~ GE_source
    ),
    DC.Unit = if_else(!is.na(DC.Value), "MJ/kg", DC.Unit)
  ) %>%
  select(-D.Item_clean, -D.Item_base, -GE_impute, -GE_min, -GE_max)


# === 7. Recombine with non-GE variables ===================================

Gross_energy <- Gross_energy %>%
  filter(DC.Variable != "GE") %>%
  bind_rows(Gross_energy_GE_imputed)

###########################################################################
## END OF ONE COMPLETE CHUNK
###########################################################################
...
Gross_energy <- Gross_energy %>%
  ...
  mutate(
    DC.Value  = if_else(
      DC.Variable == "GE" & !is.na(flag_nonenergy) & flag_nonenergy,
      0,
      DC.Value
    ),
    DC.Unit   = if_else(
...
metadata_cols <- merged_metadata$Animals.Diet.Comp %>%
  ...
  distinct()
# First, de-duplicate metadata_cols
metadata_cols_unique <- metadata_cols %>%
  group_by(B.Code, D.Item) %>%
  summarise(
    is_entire_diet = case_when(
      any(is_entire_diet == TRUE, na.rm = TRUE) ~ TRUE,
      any(is_entire_diet == FALSE, na.rm = TRUE) ~ FALSE,
      TRUE ~ NA
    ),
    Feedipedia = first(na.omit(Feedipedia)),
    .groups = "drop"
  )

# Now create ge_predictors with unique metadata
ge_predictors <- merged_metadata$Animals.Diet.Comp %>%
  ...
  left_join(metadata_cols_unique, by = c("B.Code", "D.Item"))

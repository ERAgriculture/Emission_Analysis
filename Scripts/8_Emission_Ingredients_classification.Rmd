...
```
Figure 1 shows the relationship between annual enteric CH₄ emissions and product yield for cattle, goats, and sheep, separated by meat and milk production systems. Across all species, CH₄ emissions increase with yield, indicating that higher-producing animals generally emit more methane per year. However, the magnitude and slope of these relationships differ among species. Cattle have the highest absolute CH₄ emissions, reflecting their larger size and output levels, while goats and sheep emit less overall but follow similar positive trends. The slope of increase appears steeper in goats and sheep, suggesting that small changes in productivity are associated with relatively larger changes in CH₄ output in these smaller ruminants. These species-specific patterns highlight the diversity of emission–yield dynamics captured in African feeding trials and underscore the need to interpret CH₄ responses within the biological and management context of each species.
...
# ============================================================
# 🧩 Empirical + Modeled Nutrient Composition (clean + compact)
# ============================================================

# --- 1️⃣ Extract empirical ingredient composition ----
...
# --- 2️⃣ Combine empirical and modeled data ----
...
# --- 3️⃣ Add representative "molasses" nutrient values ----
...
# Step 2️⃣ – Replace the GE value with the mean GE from *all* molasse-type ingredients
...
# Step 3️⃣ – Replace GE in the molasses table with the computed mean
molasses_means <- molasses_means %>%
  mutate(GE = molasses_GE_mean)


# Step 4️⃣ – Add or replace this entry in the full composition table
...
cat("✅ Combined empirical + modeled composition built, with 'molasses' added.\n")


cat("✅ 'molasses' nutrient values set to mean of all ingredients containing 'molasse'.\n")
...
cat("\n✅ Nutrient coverage (ingredients used in diets):\n")
...
cat("\n✅ Diet-level nutrient table built. N diets:", nrow(diet_nutrient), "\n")


# --- 11) Diet-level nutrient coverage with diagnostics ----

# 1️⃣ Compute diet coverage as before
...
cat("\n📊 Diet coverage computed for", nrow(diet_coverage), "diets.\n")

# 2️⃣ Identify incomplete diets (<100% nutrient coverage)
incomplete_diets <- diet_coverage %>%
  filter(pct_with_data < 100)

cat("\n❗ Incomplete diets found:", nrow(incomplete_diets), "\n")

# 3️⃣ Diagnose *which ingredients* are missing which nutrients
...
# 4️⃣ Summary of missing ingredients per diet
...
# 5️⃣ Display results
cat("\n🔍 Missing nutrient details (first 15 rows):\n")
print(head(diet_missing_details, 15))

cat("\n📈 Summary of incomplete diets:\n")
print(missing_summary)

# 6️⃣ Fully covered diets (100% of relevant ingredients have all nutrients)
complete_diets <- diet_coverage %>% filter(pct_with_data == 100)
cat("\n✅ Fully covered diets (relevant ingredients only):", nrow(complete_diets), "\n")
...
# --- 12) Reference diet per paper × outcome (control if present, else lowest EI) ----
...
# ============================================================
# CHUNK A — Reference diet + deltas (adds log-ratio outcomes)
...
build_ref_full_control_then_first <- function(diet_nutrient,
  ...
  # Collapse to unique diets within each study × outcome
  per_diet <- dn %>%
  ...
  ref_diets <- per_diet %>%
    ...
    dplyr::slice_head(n = 1) %>%      # exactly one reference per study × outcome
    dplyr::ungroup()
...
cat("\n✅ Delta table (with ln outcomes) built. N within-study pairs:", nrow(diet_deltas), "\n")
...
# --- 15) FIGURE 1 – Coefficient plot (compact) ----
...
# ============================================================
# CHUNK B — Models + counts (LOG outcomes; % transform)
...
model_meta <- tibble::tibble(
  outcome   = c("EI (% change)","Yield (% change)","CH₄ (% change)"),
...
cat("Obs — lnEI:", model_meta$n_obs[1],
    "| lnY:", model_meta$n_obs[2],
    "| lnCH4:", model_meta$n_obs[3], "\n")
cat("Papers — lnEI:", model_meta$n_papers[1],
    "| lnY:", model_meta$n_papers[2],
    "| lnCH4:", model_meta$n_papers[3], "\n")

# Tidy → transform coefficients to % change = 100*(exp(β)−1)
  ...
  broom::tidy(m, conf.int = TRUE) %>%
    ...
    dplyr::mutate(
      ...
      term = dplyr::recode(term,
        "dnut_ME_1"    = "ΔME (per 1 MJ/kg)",
        "dnut_NDF_100" = "ΔNDF (per 100 g/kg)",
        "dnut_CP_50"   = "ΔCP (per 50 g/kg)",
        "dnut_EE_10"   = "ΔEE (per 10 g/kg)"
      ),
      n_obs    = nrow(used),
...
coef_all <- dplyr::bind_rows(
  ...
  coef_df(m_ch4_ln, "CH₄ (% change)",   mod_ch4_data, "lnCH4")
)
...
```{r include=FALSE}
# ============================================================
# CHUNK C — Coefficient plot (from LOG models; % scale)
...
ggplot(coef_all,
  ...
  labs(
    ...
    subtitle = "Effects shown as % change per unit nutrient (100·[e^β−1])",
...
```{r include=FALSE}
# ===== A) LONG TABLE (one row per term × outcome) =====
...
# 1️⃣ Remove zero or missing nutrient deltas first
...
# 2️⃣ Compute mean absolute nutrient changes per paper × outcome
nut_delta_summary <- diet_deltas_nonzero %>%
  ...
  # 3️⃣ Summarise across all papers to get global averages and spread
  summarise(
    across(
...
# 4️⃣ Print compact table with units
cat("\n📊 Mean within-paper absolute nutrient changes (excluding zeros):\n")
...
# 5️⃣ Histogram plot (excluding zeros)
diet_deltas_nonzero %>%
...
# 1️⃣ Remove zero or missing nutrient deltas (keep only diets that changed)
diet_deltas_nonzero <- diet_deltas %>%
...
# 2️⃣ Compute within-study nutrient changes (mean per paper × outcome)
...
# 3️⃣ Aggregate across studies to global summaries (mean, sd, median, IQR)
...
# 4️⃣ Build compact summary tables
## Absolute changes
...
# 5️⃣ Print both tables
cat("\n📊 Mean absolute within-study nutrient changes (excluding zeros):\n")
print(nut_table_abs)
cat("\n📈 Mean signed within-study nutrient changes (directional, excluding zeros):\n")
print(nut_table_signed)

# 6️⃣ Optional histogram for visual check (zeros removed)
diet_deltas_nonzero %>%
  pivot_longer(c(dnut_CP, dnut_NDF, dnut_EE, dnut_ME, dnut_ADF),
               names_to = "nutrient", values_to = "delta") %>%
  filter(!is.na(delta) & delta != 0) %>%
...
# B Nutrient–Response Model Development

## Within-Study Comparative Framework

We quantify how **observed diet shifts** affect **enteric methane (CH₄), product yield (Y), and emission intensity (EI = CH₄/Y)** using a strictly **within-study** design that controls for all between-study differences (genetics, climate, management, instrumentation). For each study × outcome, we select a **single reference diet** using a transparent rule:
...
- **Percent changes**: ΔCH₄ (%) = 100·(CH₄ − CH₄\_ref)/CH₄\_ref;  
  ΔY (%) = 100·(Y − Y\_ref)/Y\_ref;  
  ΔEI (%) = 100·(EI − EI\_ref)/EI\_ref,
- **Log-ratios (recommended primary scale)**:  
  lnCH₄ = ln(CH₄/CH₄\_ref), lnY = ln(Y/Y\_ref), and  
  **lnEI = lnCH₄ − lnY** (by identity, since EI = CH₄/Y).
...
We fit ordinary least-squares (OLS) models **separately within each species × product stratum** (cattle/sheep/goat × milk/meat) to preserve species- and system-specific biology. For each stratum and each outcome, we estimate two closely related specifications:

1) **Percent-change model (Δ-scale):**  
\[
...
- **Reference selection updated:** models are built after re-anchoring each study to **CONTROL** where available, otherwise **first diet**—ensuring a consistent baseline across studies.
...
- **Panel-wise sample sizes and \(R^2\):** for every species × product × outcome panel we display the number of usable contrasts (n) and the \(R^2\), aiding assessment of fit and data support.
- **Guardrails for small n:** strata with too few complete contrasts are not plotted to avoid unstable estimates.

This linear-on-logs approach retains the **design-based control** of the within-study framework while yielding **directly interpretable elasticities**: e.g., the coefficient on ΔME gives the **percent change** in CH₄ (or Y, or EI) for a +1 MJ·kg⁻¹ DM shift in ME relative to the study’s reference diet.
...
- **One column per species**, **one row per outcome** (CH₄, EI, Yield).  
...
3) **Direct EI identity:** modeling lnCH₄ and lnY implies lnEI **by construction** (\(\ln\mathrm{EI}=\ln\mathrm{CH_4}-\ln Y\)), enhancing internal consistency.

Together, the updated **reference-aware** contrasts and the **log-ratio linear models** deliver estimates that are **comparable, interpretable, and robust** to between-study heterogeneity—ideal for extracting policy-relevant patterns from heterogeneous experimental evidence.


```{r,echo=FALSE,fig.width=12, fig.height=7, out.width='100%', fig.align='center'}
# ============================================================
# CHUNK D — Species × Product panels with R² (LOG outcomes; %)
# Adds right-hand row labels ("CH₄", "EI", "Yield") using patchwork
# Produces: final_plot
# ============================================================
...
# ---- Fit per Species × Product; return % change coefficients + R² ----
fit_one <- function(sp, prd, n_min = 20) {
  ...
  bind_rows(
    fit_outcome("lnCH4", "CH₄ (% change)"),
    fit_outcome("lnEI",  "EI (% change)"),
    fit_outcome("lnY",   "Yield (% change)")
  )
}

# ---- Run models across Species × Product ----
...
coef_species_prod <- coef_species_prod %>%
  mutate(
    outcome = factor(outcome, levels = c("CH₄ (% change)", "EI (% change)", "Yield (% change)")),
    panel   = interaction(Product, outcome, sep = " – ", drop = TRUE)
  )
...
nutrient_colors <- c(
  "ΔCP (per 50 g/kg)"   = "blue4",
  "ΔEE (per 10 g/kg)"   = "darkorange",
  "ΔME (per 1 MJ/kg)"   = "seagreen3",
  "ΔNDF (per 100 g/kg)" = "plum2"
)

# ---- Plot helper (no y-strip text; we’ll add row labels at far right) ----
...
```{r,echo=FALSE,fig.width=12, fig.height=7, out.width='100%', fig.align='center'}
# ============================================================
# CHUNK F — Milk-only panels (LOG outcomes; % transform)
# Produces: final_milk_plot
# Notes:
#   • Matches CHUNK D layout: three species panels combined horizontally
#     PLUS a single right-hand column of row labels ("CH4", "EI", "Yield")
#   • Uses dplyr::recode() explicitly (avoids car::recode clash)
#   • Coefficients are % change per reporting unit of nutrient
# ============================================================
...
```{r echo=FALSE}
# ============================================================
# UNIQUE counts per Nutrient × Outcome × Species × Product
# Counts:
#   • n_obs_unique    = number of UNIQUE observations (distinct original rows)
#   • n_papers_unique = number of UNIQUE papers (distinct B.Code)
# Method:
#   • Pivot outcomes and nutrients to long form,
#     keep rows where BOTH the chosen outcome and nutrient are non-missing,
#     then count uniques by Species × Product × Outcome × Nutrient.
# ============================================================
...
if (requireNamespace("knitr", quietly = TRUE)) {
  knitr::kable(summary_counts_unique,
               caption = "UNIQUE counts per Nutrient × Outcome × Species × Product (log outcomes)")
} else {
...
# --- Helper: fit one product × outcome with species interactions ------------
...
# --- Fit models (per Product × Outcome) ----------------------
...
summarise_model_species <- function(m, outcome_label, product_label) {
  ...
  draws_long <- bind_rows(
    one_term("dnut_ME_1",    "ΔME (per 1 MJ/kg)"),
    one_term("dnut_NDF_100", "ΔNDF (per 100 g/kg)"),
    one_term("dnut_CP_50",   "ΔCP (per 50 g/kg)"),
    one_term("dnut_EE_10",   "ΔEE (per 10 g/kg)")
  )
...
# --- Plot helper: 3×3 per Product (rows = outcomes; cols = Species) ----------
nutrient_colors <- c(
  "ΔCP (per 50 g/kg)"   = "blue4",
  "ΔEE (per 10 g/kg)"   = "darkorange",
  "ΔME (per 1 MJ/kg)"   = "seagreen3",
...
plot_product_3x3 <- function(product_name) {
  ...
  ggplot(dat,
    ...
    scale_color_manual(values = nutrient_colors) +
    labs(
      title = paste("Within-study nutrient shifts —", product_name),
    ...
    # Put an R² in the top-right of each facet (per row × col)
    geom_text(
      data = r2_lab,
...
# --- Build & print the two 3×3 figures -------------------------------------
